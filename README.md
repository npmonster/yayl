...
```sh
zig fetch --save git+https://github.com/npmonster/yayl#v0.16.0
```

That records the resolved commit and a content hash in your `build.zig.zon`, so your build stays reproducible even if the tag later moves or disappears. Leave off `#v0.16.0` and you pin whatever `main` happens to be at that moment, which is rarely what you want.
